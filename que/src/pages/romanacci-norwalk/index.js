...
const Index = () => {
  const GONATION_ID = "5730f6077455de63b3fe94dd"
  const [selectedMenu, setSelectedMenu] = useState("Food Menu")
  const options = [
    `Food Menu`,
    `ESPRESSO BAR`,
    "Drink Menu",
  ...
  const displayMenu = () =>
    menus
      ...
      .map(menu => (
        <>
          <span></span>
          {console.log(`powered-list-${menu.pl}`, GONATION_ID)}
          <Menu
            gonationID={GONATION_ID}
            key={menu.name}
